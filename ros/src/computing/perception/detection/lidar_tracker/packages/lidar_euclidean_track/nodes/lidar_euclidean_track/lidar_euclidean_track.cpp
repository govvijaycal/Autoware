...
void cluster_cb(
  ...
  for (int i(0); i < (int)base_msg.clusters.size(); ++i) {
    ...
    marker_textlabel.header.frame_id = "base_link";
...
int main(int argc, char *argv[]) {
  ...
  ros::Subscriber cluster_centroids_sub =
      n.subscribe("/detection/lidar_detector/cloud_clusters", 1, cluster_cb);
  tracked_pub =
      n.advertise<autoware_msgs::DetectedObjectArray>("/detection/lidar_track/detected_objects", 1);
  tracked_bba_pub = n.advertise<jsk_recognition_msgs::BoundingBoxArray>(
      "/detection/lidar_track/bounding_box", 1);
  tracked_bba_textlabel_pub = n.advertise<visualization_msgs::MarkerArray>(
      "/detection/lidar_track/tracked_text", 1);
  ros::spin();
