...
void transformConvexHull(const geometry_msgs::PolygonStamped &in_hull,
                          geometry_msgs::PolygonStamped &out_hull, const std::string &in_target_frame,
                          const std_msgs::Header &in_header)
{
    out_hull.header = in_header;
    out_hull.header.frame_id = in_target_frame;
   
    for(const auto& pt: in_hull.polygon.points) {
      geometry_msgs::PointStamped in_pt, trans_pt;
      in_pt.header = in_header;
      in_pt.point.x = pt.x;
      in_pt.point.y = pt.y;
      in_pt.point.z = pt.z;
      try{
        _transform_listener->transformPoint(in_target_frame, ros::Time(), in_pt, in_header.frame_id, trans_pt);
      }
      catch (tf::TransformException &ex) {
        ROS_ERROR("transformBoundingBox: %s", ex.what());
      }

      geometry_msgs::Point32 trans_pt_32;
      trans_pt_32.x = trans_pt.point.x;
      trans_pt_32.y = trans_pt.point.y;
      trans_pt_32.z = trans_pt.point.z;
      out_hull.polygon.points.push_back(trans_pt_32);
    }
}
...
void publishBoundingBoxes(const autoware_msgs::CloudClusterArray &in_clusters) {
  jsk_recognition_msgs::BoundingBoxArray bb_array_msg;
  bb_array_msg.header = in_clusters.header;
  for(const auto& cluster: in_clusters.clusters) {
    bb_array_msg.boxes.push_back(cluster.bounding_box);
  }
  _pub_bounding_boxes.publish(bb_array_msg);
}
                          ...
                          const std::string &in_target_frame, const std_msgs::Header &in_header)
{
  if (in_target_frame != in_header.frame_id)
  {
    ...
    for (auto i = in_clusters.clusters.begin(); i != in_clusters.clusters.end(); i++)
    {
      ...
      cluster_transformed.id = i->id;
      cluster_transformed.label = i->label;
      cluster_transformed.score = i->score;
      cluster_transformed.indicator_state = i->indicator_state;
      try
      {
        ...
        // Bounding Box and Conv Hull Transformation.
        transformBoundingBox(i->bounding_box, cluster_transformed.bounding_box, in_target_frame, in_header);
        transformConvexHull(i->convex_hull, cluster_transformed.convex_hull, in_target_frame, in_header);
       
        // Eigenvector rotation.
        for(const auto& evec : i->eigen_vectors) {
          geometry_msgs::Vector3Stamped in_eigenvec, trans_eigenvec;
          in_eigenvec.header = in_header;
          in_eigenvec.vector = evec;
          _transform_listener->transformVector(in_target_frame, ros::Time(), in_eigenvec, in_header.frame_id, trans_eigenvec);
          cluster_transformed.eigen_vectors.push_back(trans_eigenvec.vector);
        }
    ...
    publishBoundingBoxes(clusters_transformed);
  } else
  {
    in_publisher->publish(in_clusters);
    publishDetectedObjects(in_clusters);
    publishBoundingBoxes(in_clusters);
  }
}
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  if (!_using_sensor_cloud)
  {
    ...
    // for(const auto& cc : cloud_clusters.clusters) {
    //   const auto& bb_pos = cc.bounding_box.pose.position;
    //   const auto& bb_ori = cc.bounding_box.pose.orientation; 
    //   ROS_INFO("cluster %d: BB position (%f, %f, %f) and orientation (%f, %f, %f, %f)"
    //            " est angle: %f, conv_hull_num_pts: %d", 
    //            cc.id,
    //            bb_pos.x,
    //            bb_pos.y,
    //            bb_pos.z,
    ...
    //            bb_ori.w,
    //            cc.estimated_angle,
    //            cc.convex_hull.polygon.points.size());
    // }
...
int main(int argc, char **argv)
{
  ...
  _pub_bounding_boxes = h.advertise<jsk_recognition_msgs::BoundingBoxArray>("/detection/lidar_detector/bounding_boxes", 1);
